...
The `artifacts` directory is created by `train.py` and is loaded by `app.py` at startup.
...
After generating artifacts with `train.py`, start the web server with: 
...
- ALS hyperparameters (factors, regularization, iterations) and TF-IDF options can be tuned directly in `train.py`.  
